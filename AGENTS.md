...
- A preview proxy auto-discovers whatever you serve on **`0.0.0.0:8083`** and
  ...
  `0.0.0.0:8083`**, **verified by you**, dev server **left up**.
...
- The app **must listen on `0.0.0.0:8083`** — the preview proxy prefers a server
...
3. **Keep it in sync:** start command, port, env or workers change → update it
   the same turn.
4. **Idempotent and non-blocking:** probe `http://127.0.0.1:8083/`, exit 0 if
   healthy, start only what is down, and background it so the script returns
   fast.
5. **Bind the preview** on **`0.0.0.0:8083`**, and keep **no secrets** that
...
`http://127.0.0.1:8083`. **Always write QA screenshots under
